...
function ProblemInterFace() {
  ...
  useEffect(() => {
    if (isLoggedIn && data?.getAllSubmissionsByUser?.submissions) {
      const mysub = data.getAllSubmissionsByUser.submissions
        .filter(
          (item: Submission) =>
            item.problem.id === problem_id && item.author.id === user.id
        )
        .slice(0, 6);
      if (mysub.length > 0) {
    ...
    if (!RAPIDAPI_KEY) {
      ...
      setOutput(
        <div className="p-3 bg-red-500/10 border border-red-200 rounded-md">
          ...
          <div className="text-sm text-muted-foreground">
            <p>
              Judge0 API key is not configured. Please add it to your
    ...
    try {
      ...
      const submissions = await Promise.all(
        RunTesCases.map(async (test) => {
          ...
          try {
            ...
            if (!submissionRes.ok) {
              throw new Error(
                `API responded with status: ${submissionRes.status}`
              );
            }
        ...
        validSubmissions.map(async ({ token, input, expected }) => {
          ...
          do {
            try {
              ...
              if (!resultRes.ok) {
                throw new Error(
                  `API responded with status: ${resultRes.status}`
                );
              }
          ...
          {results.map((r, index) => (
            <div key={index} className="mb-3 p-3 border rounded-md">
              <div className="flex items-center justify-between mb-2">
                ...
                <Badge
                  variant={
                    r.actualOutput === r.expected ? "outline" : "outline"
                  }
                  className={
                    r.actualOutput === r.expected
                      ? "bg-green-500/10 text-green-500 border-green-200"
                      : "bg-red-500/10 text-red-500 border-red-200"
                  }
                >
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div>
                  <div className="text-muted-foreground">Input:</div>
                  <pre className="bg-muted/50 p-1 rounded mt-1">
                    {r.input.trim()}
                  </pre>
                </div>
                <div>
      ...
      setOutput(
        <div className="p-3 bg-red-500/10 border border-red-200 rounded-md">
          ...
          <div className="text-sm text-muted-foreground">
            <p>
              There was an error executing your code. Please try again later.
            </p>
        ...
        const res = await createSubmission({
          variables: {
            input: {
              ...
              language:
                languages.find((item) => item.id === language)?.name || "C++",
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="bg-muted p-3 rounded-md space-y-2">
                        <div>
                          ...
                          <pre className="mt-1 text-sm bg-muted/50 p-2 rounded">
                    ...
                    <div className="space-y-2">
                      ...
                      <pre className="text-sm bg-muted/50 p-2 rounded whitespace-pre-wrap">
                        {problem.constraints}
                      </pre>
                    </div>
                  </div>
                ...
                <TabsContent
                  value="solution"
                  className="flex-1 overflow-scroll  p-4"
                >
                    ...
                    {isLoggedIn ? (
                      <>
                        <button
                          ...
                          className="w-full p-3 bg-orange-300 rounded-lg hover:bg-orange-500"
                        >
                          {generating ? (
                        ...
                        {!solution && (
                          <p className="text-center text-gray-500 text-sm">
                            **you will not get the points for this problem if
                            you see solution**
                          </p>
                        )}
                        {solution && (
                          <div className="p-4 border rounded-lg bg-gray-100 dark:bg-gray-800">
                            ...
                            <div className="flex flex-col gap-2 overflow-scroll h-[50vh]">
                              {solution?.answer?.map((item) => (
                                <div
                                  key={item.language}
                                  className="border rounded-lg overflow-scroll"
                                >
                                  <button
                                    onClick={() =>
                                      toggleCollapse(item.language)
                                    }
                      ...
                      <div className="text-center">Login to see solution</div>
                    )}
                  </div>
                </TabsContent>
              </Tabs>
                ...
                <ResizablePanel defaultSize={70} className="bg-card">
                  <div className="p-4 border-b flex items-center justify-between">
                    <Select
                      ...
                      onValueChange={(val) => setLanguage(Number(val))}
                    >
                ...
                <ResizablePanel defaultSize={30} className="bg-card">
                  <div className="p-4 border-b flex items-center gap-2">
                    ...
                    {status === "running" && (
                      <Badge
                        variant="outline"
                        className="ml-2 bg-blue-500/10 text-blue-500 border-blue-200"
                      >
                        Running...
                      </Badge>
                    )}
                    {status === "success" && (
                      <Badge
                        variant="outline"
                        className="ml-2 bg-green-500/10 text-green-500 border-green-200"
                      >
                        Success
                      </Badge>
                    )}
                    {status === "error" && (
                      <Badge
                        variant="outline"
                        className="ml-2 bg-red-500/10 text-red-500 border-red-200"
                      >
                        Error
                      </Badge>
                    )}
